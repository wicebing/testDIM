...
class encoder(nn.Module):
    def __init__(self):
        ...
        self.encode1 = nn.Sequential(nn.Conv2d(16, 16, 3),
                                     nn.GELU(),                                     
                                     nn.BatchNorm2d(16),
                                     nn.Dropout(0.5),
                                     nn.Conv2d(16, 32, 3),
...
class GnLD(nn.Module):
    def __init__(self):
        ...
        self.GLD = nn.Sequential(nn.Conv2d(80, 128, 3),
                                 nn.GELU(),
                                 nn.BatchNorm2d(128),
                                 nn.Dropout(0.5),
                                 nn.Conv2d(128, 64, 3),
...
class GloD(nn.Module):
    def __init__(self):
        ...
        self.encode1 = nn.Sequential(nn.Conv2d(16, 16, 5),
                                     nn.GELU(),
                                     nn.Dropout(0.5),
                                     nn.BatchNorm2d(16),
                                     nn.Conv2d(16, 32, 5),
...
class GloD2(nn.Module):
    def __init__(self):
        super(GloD2, self).__init__()
...
class DIM(nn.Module):
    def __init__(self,device='cuda',alpha=1, beta=1, gamma=1, delta=1):
    ...
    def forward(self, 
        ...
        Lcont_0 = torch.cat([GLD0[:,:1],GLD1[:,:1]],dim=-1)
        Lcont_1 = torch.cat([GLD0[:,1:],GLD1[:,1:]],dim=-1)
        
        GLD0_loss = criterion_em(Lcont_0,true_em.view(-1).long())
        GLD1_loss = criterion_em(Lcont_1,fake_em.view(-1).long())
        ...
        Gcont_0 = torch.cat([Global0[:,:1],Global1[:,:1]],dim=-1)
        Gcont_1 = torch.cat([Global0[:,1:],Global1[:,1:]],dim=-1)
        
        Global0_loss = criterion_em(Gcont_0,true_em.view(-1).long())
        Global1_loss = criterion_em(Gcont_1,fake_em.view(-1).long())
                
        Global_loss = self.beta*(Global0_loss+Global1_loss)

        #simSLR
        emN = torch.cat([EDisease,EDisease2],dim=0)
        EMF = F.normalize(emN)
        
        sim = torch.mm(EMF,EMF.T)
        sim = sim - 1e3*torch.eye(sim.shape[0],device=self.device)
        
        target = torch.arange(2*bs,device=device)
        target = torch.cat([target[bs:],target[:bs]])
        
        simloss = self.delta*criterion_em(sim,target)
        ...
        if ptloss:
            with torch.no_grad():
                print('GT:{:.4f}, GF:{:.4f}, GloT:{:.4f}, GloF:{:.4f}, Prior{:.4f}, Sim{:.4f}'.format(GLD0_loss.item(),
                                                                              GLD1_loss.item(),
                                                                              Global0_loss.item(),
                                                                              Global1_loss.item(),
                                                                              prior_loss.item(),
                ...
                print(EDisease[0])
                print(EDisease[1])
                print('GLD0',GLD0[:3])#,true_em,true_domain)
                print('GLD1',GLD1[:3])#,fake_em,fake_domain)
                print('Global0',Global0[:3])#,true_em,true_domain)
                print('Global1',Global1[:3])#,fake_em,fake_domain)
                #print('pred_domain_T',pred_domain_T)
        return GLD_loss+prior_loss+Global_loss+simloss
